...
function SendBadge({ type }) {
  ...
  return (
    <span className={`send-badge send-badge--${badgeType}`}>
      {badgeType}
    </span>
  );
}
...
function gradeToPoints(grade) {
  ...
  if (upper in GRADE_POINTS) {
    return GRADE_POINTS[upper];
  }

  if (grade in ROPE_GRADE_POINTS) {
    return ROPE_GRADE_POINTS[grade];
  }

  const match = upper.match(/^V(\d+)/);

  if (match) {
    return GRADE_POINTS[`V${match[1]}`] || parseInt(match[1]) * 30;
  }
...
function calcSendScore(grade, attempts) {
  return Math.round(
    gradeToPoints(grade) * attemptsMultiplier(attempts)
  );
}
...
function aggregateSends(sends) {
  ...
  sends.forEach(send => {
    ...
    userMap[uid].totalScore += calcSendScore(
      grade,
      send.attempts
    );
  });
...
function ClipRow({ clip }) {
  ...
  const playableInline = isDirectVideoUrl(
    clip.video_url
  );
...
function Hero({ login }) {
  return (
    ...
    >
      ...
      >
        Track every route, clip your sends, and see
...
export default function Home() {
  ...
  const loadTop3 = async () => {
    ...
    setTop3(
      aggregateSends(sends ?? [])
    );
  };

  // Get unique walls from the clips
  const walls = [
    ...new Set(
      (clips ?? [])
        .map(clip => {
          const route =
            clip.routes || clip.route;
  ...
  // Filter clips based on selected wall
  const filteredClips =
    wallFilter === 'all'
      ? clips ?? []
      : (clips ?? []).filter(clip => {
        ...
        {tab === 'sends' && (
          <section>
            ...
            <SectionHead
              ...
              meta={
                clips
                  ? `${filteredClips.length} logged`
                  : ''
              }
            />

            {/* Wall filter */}
            {clips && walls.length > 0 && (
              <WallFilter
                walls={walls}
                selectedWall={wallFilter}
              ...
              {clips === null ? (
                ...
                <p className="text-muted text-sm">
                  {wallFilter === 'all'
                    ...
                    : `No climbs logged on the ${wallFilter}.`}
                </p>
